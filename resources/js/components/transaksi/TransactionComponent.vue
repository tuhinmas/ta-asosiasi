                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="dropdown-search">
                            <ul>
                                <li v-for="data in productSearch" :key="data.id" @click="addProductToCart(data)"><span><b>{{ data.product_id.toUpperCase() }}</b> - {{ data.product_name }}</span></li>
                            </ul>
                        </div>
                    </div>
                            ...
                            <table class="table table-striped" id="table-transaction">
                                ...
                                <tbody>
                                    <tr v-for="(product,index) in cart" :key="product.id">
                                        ...
                                        <td>
                                            <!-- <b>{{ product.product_id.toUpperCase() }}</b> -->
                                            <b>{{ product.product_name }}</b></td>
                                        <td>
                                            {{ qty[index] }}
                                            <!-- <input type="number" class="form-control w-50" @keyup="changeQuantityMinus(index)" v-model="qty[index]" value="1"> -->
                                        </td>
                                        <td>
                                            {{ product.stok.stok - qty[index] }}
                                        </td>
                                        <td>
                                            Rp {{ formatPrice(product.harga) }} <span class='text-success' v-if="product.diskon !== null">DISKON!</span>
                                        </td>
                                        <td>
                                            Rp {{ formatPrice(product.disc) }} 
                                        </td>
                                        <td>
                                            Rp {{ formatPrice((product.harga + (product.ppn/100 * product.harga))*qty[index] - (product.disc*qty[index])) }}
                                        </td>
                                        <td>
                                            <button type="button" @click="deleteCart(index)" class='btn btn-danger btn-sm'><i class="fas fa-trash"></i></button>
                                            <button type="button" @click="changeQuantityMinus(index)" class='btn btn-danger btn-sm'><i class="fas fa-minus"></i></button>
                                            <button type="button" @click="changeQuantity(index)" class='btn btn-danger btn-sm'><i class="fas fa-plus"></i></button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
    ...
    methods: {
        searchProduct() {
            ...
            if(this.search == '') {
                ...
                axios.get('/api/searchProductForTransaction', {
        ...
        addProductToCart(array){
            ...
            array.quantity = 1;
            array.realPrice = array.harga;
            if(array.diskon != null){
                array.disc = (array.diskon.diskon / 100) * array.harga
            }
            else{
                array.disc = 0;
            }
        ...
        changeQuantity(index) { 
            ...
            let price = this.cart[index].harga;
            let stock = this.cart[index].stok;
            if(qty > stock) {
        ...
        countTotalPrice() {
            ...
            this.cart.forEach((data_cart, index) => {
                let realPrice = parseInt(data_cart.realPrice - data_cart.disc);
        ...
        showKeteranganModal() {
            ...
            axios.get('/api/payment_method')
                .then(res => {
                    ...
                    console.log(this.payment_methods.id);
                    // this.payment_method = this.payment_methods.id['1'];
                });
